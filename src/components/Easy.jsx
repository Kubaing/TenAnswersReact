...
const Game = ({ mode: propMode }) => {
  ...
  useEffect(() => {
    setMaxNumber(propMode === 'easy' ? 50 : 50);
    genSolution();
  ...
  const easyClicked = () => {
    ...
    // ในฟังก์ชัน easyClicked
    setResultMessage(
    <span className="redText">
    {`เลขที่สุ่ม${guessedNumber < targetNumber ? ' : มากกว่า' :' : น้อยกว่า'} ${guessedNumber}`}
    </span>
    );
  ...
  return (
    <div className="cont">
      ...
      <h2 className="ModeC">โหมด ง่าย</h2>
      <p>จำนวนรอบที่เหลือ: {remainingAttempts}</p>
      <p id="resultMessage">{resultMessage}</p>
      <p htmlFor="userGuess">กรุณาใส่ตัวเลขที่ต้องการสุ่ม(0-50)<br /></p>
      <input
        type="text"
        id="userGuess"
        value={userGuess}
